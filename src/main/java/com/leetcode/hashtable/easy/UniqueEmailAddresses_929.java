...
//
// Related Topics Array Hash Table String 👍 1369 👎 227
/*
 O(n) Runtime: 33 ms, faster than 28.08% of Java online submissions for Unique Email Addresses.
 O(n) Memory Usage: 40.4 MB, less than 44.75% of Java online submissions for Unique Email Addresses.
*/
...
public class UniqueEmailAddresses_929 {
  public int numUniqueEmails(String[] emails) {
    ...
    for (String email : emails) {
      ...
      int plusIdx = split[0].indexOf("+");
      if (plusIdx > 0) {
        split[0] = split[0].substring(0, plusIdx);
      }

      split[0] = split[0].replaceAll("\\.", "");

      emailsSet.add(split[0] + "@" + split[1]);
    }
