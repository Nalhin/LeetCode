...
// 👍 782 👎 38
/*
 O(nlog(l)) Runtime: 14 ms, faster than 84.11% of Java online submissions for Furthest Building You Can Reach.
 O(l) Memory Usage: 49.4 MB, less than 91.74% of Java online submissions for Furthest Building You Can Reach.
*/
...
public class FurthestBuildingYouCanReach_1642 {
  public int furthestBuilding(int[] heights, int bricks, int ladders) {
    ...
    for (int i = 1; i < heights.length; i++) {
      ...
      if (pq.size() < ladders) {
        pq.add(diff);
      } else if (!pq.isEmpty() && pq.peek() < diff && bricks >= pq.peek()) {
        bricks -= pq.poll();
        pq.add(diff);
      } else {
        if (bricks < diff) {
