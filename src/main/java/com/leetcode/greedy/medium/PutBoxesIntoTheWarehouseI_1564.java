...
// 👍 139 👎 12
/*
 O(nlog(n)) Runtime: 15 ms, faster than 96.10% of Java online submissions for Put Boxes Into the Warehouse I.
 O(sort(n)) Memory Usage: 51.6 MB, less than 62.99% of Java online submissions for Put Boxes Into the Warehouse I.
*/
// leetcode submit region begin(Prohibit modification and deletion)

import java.util.Arrays;

public class PutBoxesIntoTheWarehouseI_1564 {
  public int maxBoxesInWarehouse(int[] boxes, int[] warehouse) {
    ...
    for (int i = boxes.length - 1; i >= 0 && warehousePointer < warehouse.length; i--) {
      minValue = Math.min(minValue, warehouse[warehousePointer]);
      if (boxes[i] <= minValue) {
        result++;
        warehousePointer++;
      }
