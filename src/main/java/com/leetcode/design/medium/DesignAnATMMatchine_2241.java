...
//
//
// For example, if you want to withdraw $300 and there are 2 $50 banknotes, 1 $1
// 00 banknote, and 1 $200 banknote, then the machine will use the $100 and $200
// banknotes.
// However, if you try to withdraw $600 and there are 3 $200 banknotes and 1 $50
// 0 banknote, then the withdraw request will be rejected because the machine will
// first try to use the $500 banknote and then be unable to use banknotes to
// complete the remaining $100. Note that the machine is not allowed to use the $200
...
// banknotes that will be handed to the user in the order $20, $50, $100, $200, and $50
// 0, and update the number of banknotes in the ATM after withdrawing. Returns [-1]
...
// 👍 46 👎 94
/*
  O(n) Runtime: 137 ms, faster than 100.00% of Java online submissions for Design an ATM Machine.
  O(1) Memory Usage: 50.8 MB, less than 100.00% of Java online submissions for Design an ATM Machine.
*/
// leetcode submit region begin(Prohibit modification and deletion)

public class DesignAnATMMatchine_2241 {
  static class ATM {

    private final long[] bank = new long[5];
    private final int[] amounts = new int[] {20, 50, 100, 200, 500};
    ...
    public int[] withdraw(int amount) {
      ...
      for (int i = 4; i >= 0 && amount > 0; i--) {
        long toTake = Math.min(amount / amounts[i], bank[i]);
        bank[i] -= toTake;
        result[i] += toTake;
        amount -= toTake * amounts[i];
      }
